...
(function ($) {
    ...
    $.extend(true, $.validator, {
        prototype: {
            defaultShowErrors: function () {
                ...
                $.each(this.errorList, function (index, value) {
                    ...
                    if (bsMajorVer === 3 && bsMinorVer >= 3) {
                        ...
                        var applyElement = $(value.element);

                        var replaceElement = _this.searchReplacement(value.element);

                        if (replaceElement)
                            applyElement = $('#' + replaceElement);

                        applyElement.removeClass(_this.settings.validClass)
                            .addClass(_this.settings.errorClass)
                ...
                $.each(_this.validElements(), function (index, value) {
                    var applyElement = $(value);
                    var replaceElement = _this.searchReplacement(value);
                    if (replaceElement)
                        applyElement = $('#' + replaceElement);

                    applyElement.removeClass(_this.settings.errorClass)
                        .addClass(_this.settings.validClass)
                        .tooltip(bsMajorVer === 4 ? 'dispose' : 'destroy');

                    if (_this.settings.unhighlight) {
                        _this.settings.unhighlight.call(_this, applyElement, _this.settings.errorClass, _this.settings.validClass);
                    }
                });
            },
            ...
            searchReplacement: function (element) {
                var returnval = '';
                if (this.settings.replacement && this.settings.replacement[element.name]) {
                    returnval = this.settings.replacement[element.name].missdirection;
                }
                return returnval;
            },
        },
